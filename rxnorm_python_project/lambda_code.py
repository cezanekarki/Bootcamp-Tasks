...
# Version Month date for the 
version_month = []

# Function that reads the excel for for generating headers data
def read_excel_from_s3(bucket):
    try:
        folder_path = 'excel_file/'
        excel_file_name = 'RxNorm_Header.xls'
        key = folder_path + excel_file_name
        
        local_excel_file = '/tmp/RxNorm_Header.xls'
        ...
        print(f"The excel file RxNorm_Header.xls is read from S3")
        ...
        for sheets in sheet_names:
            ...
            excel_data = sheets_data.iloc[:, 0].tolist()
            print(f"The Sheet, {sheets} is read from RxNorm_Header.xlsx,for it's header {excel_data}")
            headers_data[sheets] = excel_data
        return headers_data
        ...
        for obj in unzziped_response['Contents']:
            ...
            if file_name.endswith('.RRF'):
                try:
                    ...
                    df = pd.read_csv(file_response['Body'], sep='|', header=None)
                    df = df.iloc[:, :-1]
                    df['Code set'] = 'Rxnorm'
...
def read_and_relocate_rrf_files(s3, bucket, key):
    try:
        ...
        file_path = 'RxNorm_full_02052024/rrf'
