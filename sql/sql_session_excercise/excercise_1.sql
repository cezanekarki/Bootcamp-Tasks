...
SELECT count(*) AS total FROM training_data;


--Count total unique records
SELECT count(DISTINCT(trainer_name)) AS records FROM training_data;


--Use sum,avg,min,max aggregate function in training_data
...
-- Extract traine with aaverage duration time greater than 2
select trainer_name from training_data
group by trainer_name
having avg(training_session_duration_hours) > 2;


-- Extract trainer who have trained more than 3 student
select trainer_name from training_data
group by trainer_name
