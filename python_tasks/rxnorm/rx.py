...
#         source_bucket = event['Records'][0]['s3']['source_bucket']['name']
#         key = event['Records'][0]['s3']['object']['key']
        
#         if key.endswith('.xlsx'):
#            handle_excel_upload(source_bucket, key)
#         elif key.endswith('.zip'):
#             handle_zip_upload(source_bucket, key)

#         # Read the zip file directly from S3
#         zip_file_obj = s3_client.get_object(Bucket=source_bucket, Key=key)
#         zip_content = BytesIO(zip_file_obj['Body'].read())

#         # Perform RxNorm transformation
#         transfer_rxnorm(zip_content, source_bucket, key)
...
# def transfer_rxnorm(zip_content, source_bucket, zip_key):
#     try:
...
#                 excel_key = find_file_in_source_bucket(source_bucket, 'RxNorm_Header.xlsx')

#                 if not excel_key:
#                     print("Excel file not found in the bucket.")
#                     return

#                 excel_file_obj = s3_client.get_object(Bucket=source_bucket, Key=excel_key)
...
#             save_txt_file(df, source_bucket, sheet_name)
...
#         raise e
# def handle_excel_upload(source_bucket, key):
...
# def handle_zip_upload(source_bucket, key):
...
# def find_file_in_source_bucket(source_bucket, file_name):
#     try:
#         response = s3_client.list_objects_v2(Bucket=source_bucket, Prefix=file_name)
...
# def save_txt_file(df, source_bucket, sheet_name):
#     # Function to save a DataFrame to a text file and upload to S3
#     txt_file_content = df.to_csv(index=False, sep=',').encode('utf-8')
#     s3_client.put_object(Body=txt_file_content, Bucket=source_bucket, Key=f'allfiles/{sheet_name}.txt')
...
def save_txt_file(df, bucket, sheet_name):
    # Function to save a DataFrame to a text file and upload to S3
    txt_file_content = df.to_csv(index=False, sep=',').encode('utf-8')
    s3_client.put_object(Body=txt_file_content, Bucket=bucket, Key=f'fileuploading/{sheet_name}.txt')