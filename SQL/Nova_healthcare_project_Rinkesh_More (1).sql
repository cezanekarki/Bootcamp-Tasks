...
ARRAY_agg(DISTINCT STRUCT( c.usage_type, c.phone)) AS phones,
    ...
    array(coalesce(split(d.spoken_langauge_1 || "," || d.spoken_language_2 , ","),split(coalesce(d.spoken_language_1,d.spoken_language_2),","))) as languages,
...
FROM hea AS h
JOIN det AS d on h.id = d.id
JOIN ad as a on d.id = a.id
JOIN cont AS c on  a.id=c.id
group by
h.id,
h.insurer_id,
d.first_name,
