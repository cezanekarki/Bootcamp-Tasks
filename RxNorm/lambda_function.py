...
def read_excel_from_s3(bucket):
    try:
        ...
        # Excel file to the /tmp directory
        ...
        # Reading the Excel file into an ExcelFile object
        ...
        print(f'excel_headers dictionary for sheet {
              sheet_names[0]}: {excel_headers[sheet_names[0]]}')

    except Exception as e:
        print(f"Error occurred: {e}")


def code_set_and_version_month(zip_filename, rrf_df):
    try:
        ...
        version_month = pd.to_datetime(
            version_month, format='%m%d%Y').strftime('%Y-%m-%d')
...
def apply_header_to_rrf(file_name, rrf_df):
    ...
    if file_name in excel_headers:
        ...
        excel_headers_list = [
            header for header in excel_headers_list if header != 'SVER']
...
def update_nato_date(value):
    try:
        ...
        parsed_date = pd.to_datetime(
            value, format='%m/%d/%Y %I:%M:%S %p').date()
    except ValueError:
        try:
...
def process_date_columns(file_name, rrf_df):
    date_columns = ['VSTART', 'VEND', 'CREATED_TIMESTAMP',
                    'UPDATED_TIMESTAMP', 'LAST_RELEASED']
    for column in date_columns:
        if column in rrf_df.columns:
            if file_name == 'RXNSAB':
                ...
                rrf_df['SVER'] = pd.to_datetime(
                    rrf_df['VSTART'], format='%Y-%m-%d').dt.year.astype(str)
            ...
            for file_info in zip_ref.infolist():
                if file_info.filename.startswith(file_path) and not file_info.filename.endswith('/'):
                    ...
                    with zip_ref.open(file_info) as source_file:
                        ...
                        rrf_df = pd.read_csv(
                            file_content_io, delimiter='|', header=None)
                        rrf_df = rrf_df.iloc[:, :-1]
                        print(f"Row count before transformation: {
                              rrf_df.shape[0]}")
                        file_name = os.path.splitext(filename)[0]
                        apply_header_to_rrf(file_name, rrf_df)
                        rrf_df = process_date_columns(file_name, rrf_df)
                        code_set_and_version_month(zip_filename, rrf_df)
                        print(f"Row count of {file_name} after transformation: {
                              rrf_df.shape[0]}")
                    ...
                    # Upload the unzipped file to the 'unzipped' folder
                    unzipped_key = unzipped_folder + filename
                    s3.put_object(Bucket=bucket, Key=unzipped_key,
                                  Body=file_content)
                    print(
                        f"Unzipped file saved to: s3://{bucket}/{unzipped_key}")
    except Exception as e:
        print(f"Error occurred: {e}")
...
def lambda_handler(event, context):
    ...
    # functionto relocate rrf files from zip file
