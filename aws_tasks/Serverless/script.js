...
function loadTasks() {
    fetch("https://ljytx3d7sc.execute-api.us-east-1.amazonaws.com/dev/todos")
...
function displayTasks(tasks) {
    ...
    if (tasks?.data?.length > 0) {
        ...
        tableHeader.innerHTML =
            "<th>Name</th><th>Details</th><th>Actions</th>";
        ...
        tasks.data.forEach((task) => {
          ...
          <td class="action__buttons">
            <button onclick="openEditModal('${task.id}', '${task.name}', '${task.details}')">Edit</button>
...
function createTask() {
    ...
    if (taskTitle && taskDescription) {
        ...
        fetch("https://ljytx3d7sc.execute-api.us-east-1.amazonaws.com/dev/todo", {
            method: "POST",
            headers: {
...
function updateTask() {
    ...
    if (taskId && taskTitle && taskDescription) {
        ...
        fetch(
            `https://ljytx3d7sc.execute-api.us-east-1.amazonaws.com/dev/todo?id=${taskId}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
...
function deleteTask(id) {
    ...
    if (confirmed) {
        fetch(
            `https://ljytx3d7sc.execute-api.us-east-1.amazonaws.com/dev/todo?id=${id}`, {
                method: "DELETE",
            }
        )
            .then((response) => {
